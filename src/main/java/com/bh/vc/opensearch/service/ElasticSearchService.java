...
public class ElasticSearchService {
     ...
     */
    public String createPartsIndexes(String parts) throws JsonProcessingException {
        final ObjectMapper objectMapper = new ObjectMapper();
        final PartsPojo partsPojo = objectMapper.readValue(parts, PartsPojo.class);
        if (partsPojo != null) {
            AwsResponse response = createPartsDocument(ElasticSearchConstants.VC_INDEX,
                    ElasticSearchConstants.PARTS_DOCUMENT_TYPE,
                    partsPojo.toString(),
                    partsPojo.getPart_no().toString());
            ...
            if (response != null && (response.getHttpResponse().getStatusCode() == HttpStatus.CREATED.value()
                ...
                LOGGER.info("Successfully created parts indexex with ID: {} and title: {}", partsPojo.getPart_no(), partsPojo.getPart_desc());
                return partsPojo.getPart_no();
            }
        }
    ...
    public String createNewMovie(Movie movie) throws JsonProcessingException {
        ...
        if (json != null) {
            AwsResponse response = createDocument(ElasticSearchConstants.VC_INDEX,
                    ElasticSearchConstants.ORDERS_DOCUMENT_TYPE,
                    json,
    ...
    public String updateMovie(final Long id, Movie movie) throws JsonProcessingException, IdNotFoundException {
        ...
        final String movieToUpdate = getMovies(ElasticSearchConstants.VC_INDEX, 0, 100, null, movieQuery);
